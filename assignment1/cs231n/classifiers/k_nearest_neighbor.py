...
class KNearestNeighbor(object):
  ...
  def compute_distances_no_loops(self, X):
    ...
    #########################################################################
    #diff = np.square(self.X_train[np.newaxis,:,:]-X[:,np.newaxis,:])
    #dists = np.sqrt(np.sum(diff, axis=-1))
    inner = np.dot(X, self.X_train.T)
    tr = np.sum(np.square(self.X_train), axis=1)
    te = np.sum(np.square(X), axis=1)
    dists = np.sqrt(np.matrix(tr) + np.matrix(te).T - 2*inner)
    dists = np.array(dists) 
    #print(inner.shape, np.matrix(te).T.shape, np.matrix(tr).shape)
    #########################################################################
    #                         END OF YOUR CODE                              #
    #########################################################################
    return dists
